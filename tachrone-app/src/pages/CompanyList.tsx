...
export default function CompanyList() {
  ...
  // Sync local filter state with URL parameter
  useEffect(() => {
    if (categoryParam) {
  ...
  const [professionnelType, setProfessionnelType] = useState(categoryParam || 'Professionnel');
  ...
  const [appliedFilters, setAppliedFilters] = useState({
    city: '',
    type: categoryParam || 'Professionnel',
    rating: '0',
    verified: false
  });
  ...
  useEffect(() => {
    const fetchProfessionals = async () => {
      try {
        ...
        let type = undefined;
        if (showEntreprises && !showMaalems) type = 'entreprise';
        if (!showEntreprises && showMaalems) type = 'maalem';
        ...
        if (response.success) {
          const mapped = response.data.map((u: any) => ({
            ...
            type: u.type || u.professional_profile?.type || 'entreprise',
            category: u.category || u.professional_profile?.category || 'BTP',
            ...
            image: u.image || u.professional_profile?.profile_photo || "https://images.unsplash.com/photo-1581094794329-c8112a89af12?q=80&w=500&auto=format&fit=crop",
          }));
    ...
    fetchProfessionals();
  }, [categoryParam, showEntreprises, showMaalems]);

  // Dynamic Professions based on current selection
  const availableProfessions = useMemo(() => {
    const categories = professionals.map(item => item.category);
    
    // Ensure the current category from URL or state is in the list
    if (categoryParam && !categories.includes(categoryParam)) {
      categories.push(categoryParam);
    }
    if (professionnelType !== 'Professionnel' && !categories.includes(professionnelType)) {
      categories.push(professionnelType);
    }
    
    return Array.from(new Set(categories.filter(Boolean))).sort();
  }, [professionals, categoryParam, professionnelType]);
  ...
  const filteredPrestataires = useMemo(() => {
    return professionals.filter(item => {
      // 2. Advanced Search Logic (Multi-field & Multi-word)
      if (searchTerm) {
      ...
      if (appliedFilters.type !== 'Professionnel') {
        ...
        if (current !== target && currentType !== target) return false;
      }

      // 6. Min Rating (Applied)
      if (item.rating && item.rating < parseFloat(appliedFilters.rating)) return false;
      if (!item.rating && parseFloat(appliedFilters.rating) > 0) return false;

      // 7. Only Verified (Applied)
      if (appliedFilters.verified && !item.isBoosted) return false;

      return true;
    });
  }, [professionals, searchTerm, appliedFilters]);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-[#1E293B] mb-2">
              {categoryParam ? `Entreprises : ${categoryParam}` : "Toutes les entreprises BTP au Maroc"}
            </h1>
            <div className="flex items-center gap-2 text-[#64748B] text-xs">
              <Link to="/" className="hover:text-forest transition-colors no-underline">Accueil</Link>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm lg:sticky lg:top-24 mb-6">
              ...
              <div className="space-y-5">
                ...
                <button 
                  ...
                  className="w-full bg-forest text-white font-bold py-3 rounded-lg text-xs shadow-md shadow-green-100 hover:bg-[#2D4330] transition-all uppercase"
                >
                  Appliquer le filtre
                </button>
              </div>
          ...
          <div className="flex-1 w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedPrestataires.map(item => (
                <div key={item.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col relative cursor-pointer" onClick={() => window.location.href = `/entreprise/${item.slug}`}>
                  <div className="relative h-[240px] overflow-hidden">
                    ...
                    {item.isBoosted && (
                      <div className="absolute top-3 right-3 bg-forest text-white text-[8px] font-bold py-1 px-2.5 rounded-full flex items-center gap-1 shadow-lg">
                        <i className="ti ti-bolt text-[10px]"></i> Boosté
                      </div>
                    )}
                    ...
                    <div className="absolute bottom-3 left-3 w-12 h-12 bg-white rounded-lg shadow-lg flex items-center justify-center p-1 border border-gray-100">
                      <img src={item.image} alt="logo" className="w-full h-full object-cover rounded" />
                    </div>
                  </div>
                  
                  <div className="p-4 flex-1 flex flex-col" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <Link to={`/entreprise/${item.slug}`} className="text-sm font-bold text-navy truncate hover:text-forest transition-colors no-underline">
                        {item.name}
                      </Link>
                      <i className="ti ti-discount-check-filled text-mist text-sm"></i>
                      <div className="flex items-center gap-0.5 ml-auto">
                        <i className="ti ti-star-filled text-mist text-[10px]"></i>
                        <span className="text-[10px] font-bold text-navy">{item.rating || '4.5'}</span>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-1 mb-3">
                      <span className="bg-green-50 text-forest text-[9px] font-bold px-2 py-0.5 rounded border border-green-50">
                        {item.category}
                      </span>
                      {item.extraCategories && (
                        <span className="bg-gray-50 text-gray-500 text-[9px] font-bold px-2 py-0.5 rounded border border-gray-100">
                          {item.extraCategories}
                        </span>
                      )}
                    </div>

                    <p className="text-[11px] text-gray-500 leading-relaxed mb-4 line-clamp-3">
                      {item.description}
                    </p>

                    <div className="mt-auto flex gap-2">
                      <button 
                        onClick={() => openDevisModal({ proName: item.name })}
                        className="flex-1 bg-forest text-white text-[10px] font-bold py-2 rounded-lg flex items-center justify-center gap-2 hover:bg-[#2D4330] transition-colors shadow-sm"
                      >
                        <i className="ti ti-file-text text-xs"></i>
                        Devis
                      </button>
                      <button 
                        className="flex-1 border border-navy text-navy text-[10px] font-bold py-2 rounded-lg flex items-center justify-center gap-2 hover:bg-navy hover:text-white transition-all"
                      >
